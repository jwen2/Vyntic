"""Workflow run + tabular cell endpoints (Phase 2).

Run lifecycle:
  1. POST /deals/{deal_id}/workflows/{workflow_id}/runs — create run + queued
     cells, kick off background executor, return the run.
  2. GET /runs/{run_id}/stream — SSE channel of cell + run status events.
  3. GET /runs/{run_id} — full run + cells (REST snapshot).
  4. POST /runs/{run_id}/cancel — cancel queued cells (in-flight cells finish).
...
from app.models.workflow_run import WorkflowRun, WorkflowRunCreate
from app.services import workflow_run_store, workflow_store
from app.services.workflow_run_executor import kick_off_run, run_event_bus
...
async def create_run(
    ...
    if workflow.type != "tabular":
        raise HTTPException(
            status_code=400,
            detail="Only tabular workflows can be run in Phase 2; assistant runs land in Phase 3",
        )
    ...
    # Kick off the executor in the background — returns immediately.
