...
def create_conversation(deal_id: str, data: ConversationCreate):
...
def list_conversations(deal_id: str, workstream: str | None = Query(None)):
    deal = deal_store.get_deal(deal_id)
    if not deal:
        raise HTTPException(status_code=404, detail=f"Deal '{deal_id}' not found")
...
@router.delete("")
def clear_conversations(deal_id: str):
    deal = deal_store.get_deal(deal_id)
    if not deal:
        raise HTTPException(status_code=404, detail=f"Deal '{deal_id}' not found")
