...
export function sseStream<T>(
  ...
  if (isDemoMode()) {
    ...
    void import("@/demo/fixtures/chat").then(({ demoSseStream }) => {
      if (controller.signal.aborted) return;
      const inner = demoSseStream(url, body, {
        // `event` is unknown here, exactly as the parsed payload is on the live
        // path below — this is the same boundary cast, not a new one.
