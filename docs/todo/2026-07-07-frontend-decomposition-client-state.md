...
> **RESOLVED (Stanley):** D1 → (a) render typed cells directly; D2 → (a) backend tables + routes; D3 → (a) one theming system (CSS vars + Tailwind). All three took the recommended option.
>
> **F3.3 refinement 2026-07-11 (Stanley):** the KV + list cells are the genuinely lossy part of FE6 (typed JSON → fake markdown → regex). The financial-highlights and investment-thesis cells are still *prose*, so their parsers (`extractMetrics`/`extractFinancialTables`/`extractThesisSections`) aren't a lossy round-trip and deleting them would drop the metric tiles / financial tables / thesis breakdown. Chose: switch the KV+list panels to read `answer_formatted` directly and delete only `synthesizeBriefAnswer` + `extractFields` + `insertFieldBreaks` (and rewrite `extractActionItems` → `deriveActions`); **keep the three prose parsers** (full retirement needs the backend to re-type those columns — separate work). The `DealBriefDashboard` god-component *decomposition* (create `brief/` panels) is deferred: Option A kept the existing panels, so it's separable FE5 work.
