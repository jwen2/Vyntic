...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 in progress**: 15 of ~23 file-groups converted (`1e13ddf` latest), `ddTheme` still in 6 files — only 3 real files left, the top-level pages (`DealWorkspacePage.tsx`, `ManagerPage.tsx`, `PortfolioPage.tsx`); `DealBriefDashboard.tsx` stays excluded (FE5), `types.ts`/`index.css` await final shim deletion. Sweep still not mechanical; colorway goal already met by F3.5. DS3 not started |
