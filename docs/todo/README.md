...
| CS-B | not yet written | Cost: context caching, column batching **only if** the caching residual justifies breaking the per-cell claim/SSE/retry model | CS-A (spike answer + baseline) | **measured — batching is not justified.** `docs/superpowers/spikes/2026-07-30-cache-scaling-and-citation-baseline.md`: at a 71K-token prefix the implicit cache holds **92%** in steady state (12-column sweep, run `24a6166f`), so batching would break the atomic claim, per-cell SSE, per-cell retry and semaphore dispatch to reclaim tokens already cached at ~10× lower cost. Remaining lever is the **cold window** — 4 of 12 calls paid full price before any cache existed — which is dispatch ordering, not architecture |
...
**Baseline taken 2026-07-30** (`docs/superpowers/spikes/2026-07-30-cache-scaling-and-citation-baseline.md`). The citation golden set is `backend/evals/data/brightwater_golden_set.json` — 44 questions over 9 documents / 95 pages of the seeded Brightwater/Glenmoor funds. It scores **hit_rate 1.000, mean_precision 0.955, no_citation_rate 0.000**. A hit rate pinned at 1.000 is a *regression detector only*: it can prove the CS-C allocator does not degrade citation accuracy, but it can never show that a retrieval change helps. The corpus is script-generated (uniform formatting, one LPA section per page, no OCR noise), so the figure is a ceiling and must not be quoted as production accuracy. Making it discriminating needs harder documents — see the spike's closing section.
...
| CS-A5 | ~~**`cache_read` accumulation semantics still unverified**~~ — **effectively resolved 2026-07-30.** Across 8 warm calls in run `24a6166f`, `cached_tokens` was exactly 65,503 while completion length varied 53→449 tokens; per-chunk accumulation would have summed higher on longer outputs. It is a single report, so summed `cached_tokens` can be trusted. | Not formally conclusive — 449 tokens is still a short stream — but the accumulation hypothesis is falsified for this path. A genuinely long-output call would close it airtight. |
