...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 in progress**: `tabular-run/` (`deeae24`) + `cells/` (`2e3c746`) + dd/workflows dialogs (`b8d6b9a`) + TopBar/LeftSidebar (`3ab3015`) + WorkflowCard/WorkflowLibrary (`667a757`) + MemoOutput (`fce8321`) + CompareView (`92fda39`) + TabularRun (`31be933`) + TabularEditor (`f013d8b`) + AssistantEditor (`35cda88`) converted, 10 of ~23 file-groups, `ddTheme` still in 11 files (8 excl. `types.ts`/`index.css`/`DealBriefDashboard.tsx`). Sweep still not mechanical (~75% of sites need whole-object rewrites); colorway goal already met by F3.5, so continuing is a judgment call. DS3 not started |
