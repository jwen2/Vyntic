...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 in progress**: 13 of ~23 file-groups converted (`36a795d` latest), `ddTheme` still in 8 files (5 excl. `types.ts`/`index.css`/`DealBriefDashboard.tsx`). `components/workflows/` and `DealAssistantPanel.tsx` clean. Remaining: `MonitoringPanel.tsx`, `DocumentDetailView.tsx` (dead code), 3 top-level pages. Sweep still not mechanical; colorway goal already met by F3.5, so continuing is a judgment call. DS3 not started |
