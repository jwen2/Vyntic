...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 in progress**: `tabular-run/` (`deeae24`) + `cells/` (`2e3c746`) converted, 2 of ~23 file-groups, 21 left (`ddTheme` still in 23 files) — the sweep is not mechanical (~75% of sites need whole-object rewrites) and the colorway goal is already met by F3.5, so continuing is a judgment call, not a requirement. DS3 not started |
