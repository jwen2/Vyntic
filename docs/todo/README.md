...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 done** — 18 file-groups converted (`5fd8255` final); its Step 3 (delete the shim) was unreachable until the brief was decomposed and is now **closed by FE5.6** — zero `ddTheme`/`DD_LIGHT`/`DD_DARK` left in `frontend/src`. **DS3 done, brief-scoped**: DS3a (`SectionLabel` → `components/ui/`, 6 duplicates deduped) + DS3b (`--focus` token, an a11y fix — `blue-400` resolves to the dark accent in both themes, 2.09:1 on white) on `feat/design-system-ds3`; the Card primitive shipped separately on `feat/design-system-card` — **see DS-Card below**. App-wide Card rollout and Input remain deferred pending a look decision |
| RS1 | `2026-07-26-oxblood-reskin-phase1.md` | Colour reskin to the oxblood/ivory system: curated 8-tone badge palette, contrast fixes (`--text-3`, `--border`), `--violet`→ink, retokenize `theme.ts`, palette-drive `stageBadges`/`matrixColumnConfig`, de-hardcode 5 app surfaces, check in the off-palette scanner | DS3 (PR #118, merged) | **not started** — branch `feat/reskin-oxblood`, base layer at `03e6a9d`. Spec `docs/superpowers/specs/2026-07-26-oxblood-reskin-design.md` (6 decisions, D1 revised to admin-configurable badges). Done when `npm run scan:palette` exits 0 on all app routes in both themes. **Phase 2 (typography: Playfair/Hanken/DM Mono + scale) planned separately after this lands** — its decomposition depends on measured reflow. Badge persistence + picker UI are a separate feature plan |
