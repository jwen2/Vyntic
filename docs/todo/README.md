...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 done** — 18 file-groups converted (`5fd8255` final); `ddTheme` now called only from `DealBriefDashboard.tsx` (excluded, FE5 will decompose it) + its own shim definition. Cleared both pre-existing lint errors on the branch as a byproduct. **DS3 done, re-scoped** — its own Step-1 inventory moved it off Card: 62 of ~75 card-shaped borders live in the excluded `DealBriefDashboard` (~83%), and there is no consistent card radius to standardize onto (17 distinct `rounded-*` values). Input was evaluated as a substitute and rejected for the same reason (5 distinct treatments). Shipped instead: **DS3a** `SectionLabel` 6 defs → 1 primitive + `mono` variant, zero visual change (A/B computed-style diff, 9 properties, both themes) and **DS3b** `--focus` token — an a11y fix, since `blue-400` is retuned to `#8ab4ff` in *both* themes and measured 2.09:1 on white, under the 3:1 WCAG 2.2 floor. **Card + Input deferred to one deliberate geometry pass after FE5**, where they absorb ~75 sites instead of ~13 and the radius scale is decided once |
