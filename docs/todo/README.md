...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 done** — 18 file-groups converted (`5fd8255` final); its Step 3 (delete the shim) was unreachable until the brief was decomposed and is now **closed by FE5.6** — zero `ddTheme`/`DD_LIGHT`/`DD_DARK` left in `frontend/src`. **DS3 partially done** on unmerged local branch `feat/design-system-ds3`: DS3a (`SectionLabel` → `components/ui/`, 6 duplicates deduped) + DS3b (`--focus` token, an a11y fix — `blue-400` resolves to the dark accent in both themes, 2.09:1 on white). Card was re-scoped out at the time (62 of ~75 sites locked in `DealBriefDashboard`); FE5 dissolved that blocker → **Card now planned brief-scoped, see DS-Card below**. Input still deferred |

| DS-Card | `2026-07-25-card-primitive.md` | `<Card level tone>` primitive (`components/ui/`) + `--card-hero-shadow` token; migrate the 18 card containers in `brief/` | FE5.6 | not started — spec approved (`docs/superpowers/specs/2026-07-25-card-primitive-design.md`), branch `feat/design-system-card`. Brief-scoped by decision: the app's other surfaces are a tighter radius geometry (6–16px vs 18–28px) and unifying them is a look decision, not a refactor |
