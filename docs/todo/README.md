...
| DS | `2026-07-24-design-system-primitives.md` | Modal primitive (`components/ui/`) → `ddTheme(theme)` retirement (**775** refs, not 102) → Card/Panel primitive | UI1/UI2 pattern, F3.5 tokens | **DS1 done** (Modal, 5 dialogs, verified light+dark). **DS2 in progress**: `tabular-run/` + `cells/` + dd/workflows dialogs + TopBar/LeftSidebar + WorkflowCard/WorkflowLibrary + MemoOutput + CompareView + TabularRun + TabularEditor + AssistantEditor + AssistantRun (`e7a64af`) converted, 11 of ~23 file-groups, `ddTheme` still in 10 files (7 excl. `types.ts`/`index.css`/`DealBriefDashboard.tsx`). **All of `components/workflows/` clean except `WorkflowsView.tsx`** (the hub). Sweep still not mechanical; colorway goal already met by F3.5, so continuing is a judgment call. DS3 not started |
